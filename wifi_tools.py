...
BLUEDUCKY_DIR = "/home/ubuntu/BlueDucky"
...
class WifiTools:
    ...
    def install_all_tools(self):
        ...
        # Liste der Tools für Wifite2 und BlueDucky
        commands = [
            "sudo apt-get update",
    ...
    def run_blueducky(self):
        ...
        # BlueDucky benötigt oft interaktive Eingaben, daher beenden wir curses kurzzeitig
        curses.endwin()
    ...
    def attack_deauth(self, network):
        ...
        cap_file = f"{CAPTURE_DIR}/{network['essid']}_handshake"
    ...
    def attack_pmkid(self, network):
        cap_file = f"{CAPTURE_DIR}/{network['essid']}_pmkid.pcapng"
    ...
    def attack_passive(self, network):
        cap_file = f"{CAPTURE_DIR}/{network['essid']}_passive"
        airodump = subprocess.Popen(
            f"sudo airodump-ng -c {network['channel']} --bssid {network['bssid']} -w {cap_file} {MONITOR_INTERFACE}",
            shell=True, preexec_fn=os.setsid
        )
