...
def check_root():
    """Prüft auf Root-Rechte vor dem Start von Curses."""
    if os.geteuid() != 0:
        print("\n" + "="*50)
        print(" FEHLER: Dieses Skript muss mit 'sudo' ausgeführt werden!")
        print(" Beispiel: sudo python3 wifi_tools.py")
        print("="*50 + "\n")
        sys.exit(1)
...
if __name__ == "__main__":
    # 1. Root-Prüfung VOR Curses
    check_root()
    # 2. Verzeichnisse vorbereiten
    setup_directories()
    # 3. Curses Wrapper starten
    try:
        curses.wrapper(main)
